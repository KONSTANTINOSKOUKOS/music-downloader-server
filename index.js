...
const api = new Spotify({
    ...
    redirectUri: 'https://music-downloader-server.vercel.app/callback'
});
...
app.get('/callback', (req, res) => {
    api.authorizationCodeGrant(req.query.code).then(({ body }) => {
        console.log(`tokens:\n${body.access_token}\n${body.refresh_token}`);
        res.send(`<h1>Access granted, You can now close this tab</h1><script>window.spotify = '${body.access_token}'</script>`);
